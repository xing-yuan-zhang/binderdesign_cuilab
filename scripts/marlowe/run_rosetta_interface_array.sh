#!/usr/bin/env bash
# BinderDesign-CUILab stage wrapper for Rosetta InterfaceAnalyzer sanity filtering.

#SBATCH --job-name=ros_iface
#SBATCH --nodes=1
...
"$BINDERDESIGN_CUILAB_PYTHON" -m binderdesign_cuilab.cli task rosetta-interface \
  --manifest "$MANIFEST" \
